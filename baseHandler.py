...
class BaseHandler:
    ...
    def __init__(self, stop_event, queue_in, queue_out, setup_args=(), setup_kwargs={}):
        # the stop event attribute is the threading.Event object, it's used to signal when to stop the processing loop
    ...
    def setup(self):
        """set it up later on for initialization of handler-specific resources, for example, load a model"""
        pass
    ...
    def min_time_to_debug(self):
        """track processing times for each input"""
        return 0.001

    def cleanup(self):
        """Ensure proper cleanup when the handler stops, for example: releasing the resources"""
        pass
